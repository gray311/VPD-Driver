...
def generate_realtime_data(filename):
    video_path = f"./workspace/data/vpd-sft/drivelm/{filename}"
...
if __name__ == "__main__":
    ...
    with open("vpd_drivelm200k_test.json", "r") as f:
        data = json.load(f)
    ...
    for line in tqdm(data):
        ...
        for i, objects in enumerate(object_list):
            for item in objects[list(objects.keys())[0]]:
                if item['id'] not in instance_id2color.keys():
                    instance_id2color[item['id']] = color_list[item['id']]
                    instance_id2type[item['id']] = item['Visual_description']
        ...
        for frame_idx, img_path in enumerate(image_list):
            ...
            objects = object_list[frame_idx][list(object_list[frame_idx].keys())[0]]
            width, height = image.size
            boxes, labels, locations = defaultdict(list), defaultdict(list), defaultdict(list)
            segments = defaultdict(dict)
            sample_token = None

            input_boxes = [item['2d_bbox'] for item in objects]
            input_labels = [item['id'] for item in objects]
