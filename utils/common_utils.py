...
ROAD_OBJECTS = ["construction", "pedestrian", "trailer", "truck", "bus", "motorcycle", "bicycle", "car", "vehicle"]
...
def draw_number_on_bbox(image, bbox, number, font_scale=0.7, thickness=2, offset=0):
    ...
    center_x = (x1 + x2) // 2 + offset
    center_y = (y1 + y2) // 2 + offset
...
class CommonUtils:
    ...
    def draw_masks_and_box_with_supervision(raw_image_list, mask_path, json_path, output_path,  colors, types, overwrite=False):
        for frame_idx, raw_image in enumerate(raw_image_list):
            ...
            if os.path.exists(output_image_path): continue
